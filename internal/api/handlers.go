...
func SetupRoutes(router *gin.Engine, linkService *services.LinkService) {
	...
	if ClickEventsChannel == nil {
		...
		ClickEventsChannel = make(chan models.ClickEvent, 1000) // TODO: utiliser cfg.Analytics.BufferSize
	}
...
func RedirectHandler(linkService *services.LinkService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		clickEvent := models.ClickEvent{
			LinkID:    link.ID,
			IP:        c.ClientIP(), // Remplacez 'IP' par le nom correct du champ dans votre struct ClickEvent
		...
		default:
			// Channel plein, on ignore l'événement
